...
export const dialogScenariosTable = pgTable("dialog_scenarios", {
   ...
   */
  goal: text("goal"),
  /** turns — по числу реплик, goal — по достижению цели, both — что раньше. */
  finishMode: text("finish_mode").notNull().default("turns"),
  /** Сколько реплик ученика нужно. Учитывается в режимах turns и both. */
  turnsTarget: integer("turns_target").notNull().default(20),
  /**
