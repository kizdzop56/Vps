...
//                       по журналу: рейтинг спрашивается на каждом открытии
...
// попадания, дневное задание и сундуки, тратятся на атаки, энергию и свитки
// подсказки.
// ─────────────────────────────────────────────────────────────────────────────
import {
...
export const raidParticipantsTable = pgTable("raid_participants", {
  ...
  /** Сколько вех личного вклада уже выдано (шкала вех пока скрыта в клиенте). */
 ...
 *    ввод 25, сборка 50). Раз способ выдал сервер и записал сюда, клиент не может
 *    прислать «я отвечал сборкой» на задание с вариантами.
 *
 * Здесь же гасится повторная отправка: answered_at заполняется на первом
 * ответе, второй по тому же заданию урона не даёт. Отсюда же берётся эталон для
 * ПОДСКАЗКИ (см. buildRaidHint): клиент правильного ответа не знает вовсе, поэтому
 * частичную подсказку собирает сервер.
 *
...
export const raidTasksTable = pgTable("raid_tasks", {
   ...
   * build у слов; choice | type | assemble у грамматики. От него зависят и эталон
   * ответа, и ставка урона.
   */
...
export const raidStateTable = pgTable("raid_state", {
  ...
  /**
   * Мощный удар заряжен. ЛЕГАСИ: оставлена только ради старых строк, где баф
   * уже был куплен до появления счётчика ниже. Новые покупки пишут powerStacks.
   */
  powerArmed: boolean("power_armed").notNull().default(false),
  /**
   * Сколько мощных ударов куплено и ещё не потрачено.
   *
   * Было булевым флагом, и из-за этого второй мощный удар нельзя было купить,
   * пока не потрачен первый: флаг уже стоял, и покупка отвечала «уже
   * заряжен». Счётчик позволяет накопить запас впрок: каждый верный ответ
   * тратит ровно один.
   */
  powerStacks: integer("power_stacks").notNull().default(0),
  /** Сколько заданий осталось под удвоением. Покупка добавляет к остатку. */
  aoeLeft: integer("aoe_left").notNull().default(0),
  /**
   * Свитки подсказки: сколько куплено и ещё не потрачено в бою.
   *
   * Один свиток = одна подсказка на задание: лишний неверный вариант убран
   * или открыта часть букв. Полного ответа подсказка не даёт никогда — см.
   * buildRaidHint в api-server/src/lib/raidSession.ts.
   */
  hints: integer("hints").notNull().default(0),
  /** Щит от штрафа за пропуск дней. Повторная покупка продлевает срок. */
