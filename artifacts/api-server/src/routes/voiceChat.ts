...
// Раньше маршрут напрямую знал про OpenAI: и модель ответа, и whisper, и tts.
// Стоило понадобиться другому поставщику — и правка задевала проверку прав,
// очки и журнал, которые к ИИ отношения не имеют.
//
// Теперь всё это за общим слоем (lib/ai.ts): маршрут просит «ответь»,
// «расшифруй», «озвучь». Кто именно это сделал — Gemini, Deepgram или OpenAI —
...
router.get("/voice-chat/status", requireAuth, async (req, res) => {
  ...
  if (!hasAnyAi()) {
    res.json({
      ready: false,
      providers,
      reason: "Не задан ни один ключ: нужен GOOGLE_AI_API_KEY (или DEEPGRAM_API_KEY / OPENAI_API_KEY)",
    });
    return;
  }
