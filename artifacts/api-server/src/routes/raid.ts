...
//
// Урон по боссу наносится ТОЛЬКО отсюда (POST /raid/answer). Раньше любой
// верный ответ в разделе «Учёба» (слова, формы глаголов, времена) тоже бил
// босса — перехватчик routes/raidHook.ts подслушивал их ответы. Это отключено:
// «Учёба» и «Рейд» — разные занятия, и удар должен приходить только из
// заданий самого боя, иначе учить слова и участвовать в рейде становится
// одним и тем же действием без разницы для ученика.
// ─────────────────────────────────────────────────────────────────────────────
