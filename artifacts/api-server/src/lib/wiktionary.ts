...
/** Все русские переводы слова во всех его значениях. */
export function allTranslations(senses: WordSense[]): string[] {
  const out: string[] = [];
  for (const sense of senses) {
    for (const t of sense.translationsRu) if (!out.includes(t)) out.push(t);
  }
  return out;
}
...
/**
 * Знает ли словарь перевод карточки среди переводов этого слова.
 *
 *   "yes"     — перевод есть, карточка верна;
 *   "no"      — у слова есть русские переводы, но нашего среди них нет;
 *   "unknown" — словарь не дал русских переводов, судить не о чем.
 *
 * Третий вариант нужен обязательно: у Викисловаря переводы заполнены не
 * везде, и молчание словаря — это не приговор карточке.
 */
export function verdictFor(
  senses: WordSense[],
  translationsRu: string[],
): "yes" | "no" | "unknown" {
  const known = allTranslations(senses);
  if (known.length === 0) return "unknown";
  return findSense(senses, translationsRu) ? "yes" : "no";
}
