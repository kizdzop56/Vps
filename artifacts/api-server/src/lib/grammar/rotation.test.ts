 ...
 *
 * Планки по временам и сборке подняты после размножения заготовок по лицам (см.
 * expandUnits в generate.ts): банк вырос вчетверо, и прежние шесть занятий
 * перестали ловить даже трёхкратный обвал. Считать их «на глаз побольше» нельзя
 * по обратной причине — тест, который падает от каждой правки данных, вычёркивают
 * из головы, а не чинят.
 *
