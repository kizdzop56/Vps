...
// граница слова знает только латиницу, цифры и подчёркивание, поэтому /\bне\b/
// не совпадает никогда и такая «проверка» молча проходит на чём угодно. Для
// кириллицы пишем границу руками: (^|\s).
...
test("в заготовке нет счётчика: он ломает вопрос", () => {
  // «Has she read this book three times?» — и по-английски странно, и по-русски
  // вопрос выходит про другое, потому что перевод у вопроса тот же, что у
  // утверждения.
  for (const u of SENTENCE_UNITS) {
    assert.equal(
      /(twice|three times|four times)/.test(u.rest),
      false,
      `${u.id}: в хвосте счётчик`,
    );
  }
});
...
test("в отрицании есть not, и «не» есть в переводе", () => {
  // Кириллица и \b несовместимы, см. шапку файла: границу пишем руками.
  for (const t of TENSE_TASKS) {
...
test("в утверждении «не» в переводе не появляется", () => {
  for (const t of TENSE_TASKS) {
    if (t.form !== "affirmative") continue;
    assert.equal(/(^|\s)не\s/.test(t.ru), false, `${t.id}: утверждение переведено отрицанием`);
  }
});
...
test("на каждую заготовку приходится по нескольку предложений на сборку", () => {
