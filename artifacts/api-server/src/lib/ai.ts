...
//                ключ проекта; Deepgram спасает записи из браузера.
//   ОЗВУЧКА    — Deepgram, затем Gemini. Именно в таком порядке, см. ниже.
//
...
//     Safari;
//   • Deepgram Aura-2 уже озвучивает слова в карточках, то есть у Снежи
...
//
// ── ГРАБЛИ, ИЗ-ЗА КОТОРЫХ ОТВЕТЫ ОБРЫВАЛИСЬ НА ПОЛУСЛОВЕ ───────────────────
// Предел вывода стоял 400 токенов. Для трёх коротких фраз этого с запасом —
// пока модель не начала ДУМАТЬ.
//
// У Gemini 3.x внутренние размышления считаются в тот же лимит, что и текст.
// Модель тратит на них сотни токенов, натыкается на предел и отдаёт то, что
// успела: обрубок фразы, а иногда пустой ответ с finishReason MAX_TOKENS.
// Снаружи это выглядит как «оборвала на полуслове» и никак не связывается с
// числом в конфиге.
//
// Поэтому предел поднят до CHAT_MAX_TOKENS. Ответ от этого длиннее не станет —
// его длину задаёт задание («1-3 короткие фразы»), а не потолок. Потолок
// нужен только чтобы модели было где подумать.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Потолок вывода для ответа собеседницы.
 *
 * Щедрый намеренно: в него входят и размышления модели (см. ГРАБЛИ в шапке).
 * Длину реплики держит задание, а не это число.
 */
...
function modelRank(id: string): number {
  ...
  // Lite дешевле, но заметно слабее держит разговор.
 ...
 * mp4, и по названию формат определять нельзя. null — сигнатура незнакомая,
 * тогда решает заявленный тип.
 */
  ...
  for (const model of models) {
    ...
    try {
      const resp = await fetch(`${GEMINI_API}/models/${model}:generateContent`, {
        ...
        body: JSON.stringify({
          // Роль задаётся отдельным полем, а не первой репликой: иначе модель
          // начинает считать её частью разговора и пересказывать.
          system_instruction: { parts: [{ text: system }] },
          contents,
          // maxOutputTokens щедрый намеренно: в него входят размышления модели
          // (см. ГРАБЛИ в шапке файла). Длину реплики держит задание.
          generationConfig: { temperature: 0.7, maxOutputTokens: CHAT_MAX_TOKENS },
        }),
      ...
      // Пустой ответ бывает при срабатывании фильтров безопасности и при
      // упирании в предел вывода: причина лежит в finishReason, и без неё это
      // выглядит как молчание без повода.
    ...
    try {
      const resp = await fetch(`${GEMINI_API}/models/${model}:generateContent`, {
        ...
        body: JSON.stringify({
          ...
          generationConfig: { temperature: 0, maxOutputTokens: CHAT_MAX_TOKENS },
        }),
      ...
      if (!resp.ok) {
        const detail = await httpDetail(resp);
        attempts.push({ model, detail });
        // Формат записи может оказаться не из списка поддерживаемых. Пишем его
        // рядом с ошибкой, иначе причину придётся угадывать.
        log.warn(
 ...
 *
 * Gemini отдаёт PCM без всякой обёртки: ни браузер, ни expo-av такой файл не
 * проиграют, они не знают ни частоты, ни разрядности. Сорок четыре байта
 ...
 * потерянная реплика. Ученик может попросить озвучку повторно
