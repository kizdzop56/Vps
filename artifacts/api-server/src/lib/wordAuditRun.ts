...
//
// ── Что можно писать в базу без спроса ──────────────────────────────────────
// Проверено на живых данных: аудит заменил хороший пример «The soup was
// absolutely delicious» на «The irony is delicious!» и стёр «She is cooking in
// the kitchen». Правила исправлены, но вывод глубже: ЛЮБАЯ проверка значения
// иногда ошибается, а цена ошибки здесь — уничтоженный хороший пример, которого
// не вернуть.
//
// Поэтому право записи сужено до доказуемо безопасного:
//
//   • примера НЕТ → подставляем найденный. Хуже пустоты не сделаем;
//   • слова в примере нет вовсе → чиним (замена или удаление);
//   • пример о ДРУГОМ ЗНАЧЕНИИ → только отчёт. Ни замены, ни удаления, пока
//     человек не разрешит явно (replaceExamples).
//
// ── Чему нельзя верить ──────────────────────────────────────────────────────
//   • ИДИОМЫ. «A piece of cake» — «проще простого», а машина даёт «кусок
//     торта». Словосочетания автоматически не правятся никогда.
//   • МНОГОЗНАЧНОСТЬ. У «tie» есть галстук, ничья и «связывать». Сверяемся со
//     всей словарной статьёй, а не с одним «главным» переводом.
// ─────────────────────────────────────────────────────────────────────────────
...
export type AuditFinding = {
  ...
  freshRu: string | null;
  /** Все известные значения слова — показываем в отчёте для контекста. */
  senses: string[];
  ...
  badExample: boolean;
  /** Слово в примере есть, но значение похоже на чужое. Только отчёт. */
...
export type AuditScope = {
  ...
  /**
   * Разрешить замену СУЩЕСТВУЮЩИХ примеров при расхождении значения.
   * По умолчанию выключено: см. «Что можно писать в базу» в шапке файла.
   */
 ...
 * и про ничью в игре. Поэтому кандидат проходит два сита: слово должно
 * присутствовать в самой фразе, а перевод фразы — содержать перевод карточки.
 *
...
export async function inspectWord(word: AuditWordRow, scope: AuditScope = {}): Promise<AuditFinding> {
  ...
  // Замену ищем только там, где её разрешено применить: за примером о другом
  // значении лезем в словарь лишь при явном replaceExamples. Иначе это лишний
  // запрос ради строчки отчёта.
  const needsExample =
    finding.missingExample ||
    finding.badExample ||
    (finding.senseMismatch && scope.replaceExamples === true);

  // ── Словарь: примеры и часть речи ─────────────────────────────────────────
  // У идиом пример не подбираем: проверить его значение всё равно нечем, а
  // подставить дословный смысл хуже, чем оставить пусто.
 ...
 *
 * С примерами осторожнее всего (см. шапку файла):
 *   • отсутствующий — подставляем найденный;
 *   • без изучаемого слова — заменяем или стираем;
 *   • о другом значении — НЕ ТРОГАЕМ, пока не разрешено replaceExamples.
 */
export function patchFor(finding: AuditFinding, scope: AuditScope = {}): WordPatch | null {
  ...
  const mayReplace = finding.badExample || (finding.senseMismatch && scope.replaceExamples === true);

  if (finding.missingExample && finding.newExample) {
    ...
    if (finding.newExample) {
      ...
      // Замены нет: пример без изучаемого слова хуже пустоты, стираем вместе с
      // русским переводом — без английского он бессмыслен.
