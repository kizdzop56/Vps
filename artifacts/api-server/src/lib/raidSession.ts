...
// На слове help ученик написал «помощь», и ответ не приняли: в карточке стоит
// «помогать». Ученик прав, приложение спорит. Причина не в проверке, а в
// вопросе: у одного английского слова несколько частей речи, и голое слово в
// качестве задания НЕ ОПРЕДЕЛЯЕТ, какой ответ ждут.
//
// Поэтому письменные задания боя показывают ПРЕДЛОЖЕНИЕ с пропуском, а под ним
// перевод предложения; вписать надо пропущенное слово:
//
//   Can you ____ me, please?     ·  Ты можешь мне помочь?     → help
//
// Само правило подстановки пропуска живёт в lib/wordExercise.ts (gapSentence):
// та же механика нужна и тренажёру «Учёбы», а две копии одного регулярного
// выражения неизбежно разъедутся. Здесь только применение.
//
// Свободного перевода одним словом в рейде нет вовсе. Там, где предложения у
...
/** Фраза с пропуском по примеру карточки. null — примера нет или форма другая. */
function gapFor(word: WordRow): string | null {
  return gapSentence(word.exampleEn ?? "", word.english);
}
