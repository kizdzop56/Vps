...
test("build: даём перевод, ответ — слово, плитки перемешаны", () => {
  // Уровни 4–5 чередуют сборку, письмо и речь, поэтому берём слово, которое ни
  // написать целиком, ни произнести нельзя… а собрать можно: apple при wordId,
  // попавшем на сборку. Здесь проще проверить саму сборку по типу упражнения.
  const level4 = [4, 5]
    .flatMap((level) => POOL.map((w) => buildExercise({ word: w, memoryLevel: level, isNew: false, pool: POOL, now: NOW })))
    .filter((ex) => ex.type === "build");
  assert.ok(level4.length > 0, "на уровнях 4–5 сборка обязана встречаться");
  for (const ex of level4) {
...
test("без перевода, но с примером — спрашиваем фразой с пропуском", () => {
  ...
  const ex = buildExercise({ word: noTranslation, memoryLevel: 4, isNew: false, pool: [noTranslation], now: NOW });
  assert.equal(ex.type, "typeEn");
  assert.ok(ex.prompt.includes("____"));
  assert.deepEqual(ex.accept, ["help"]);
});
