...
// работало, но у раздела появляются соседи — конструктор предложений,
// неправильные глаголы, времена, — и деть их было некуда: либо кнопка среди
// колод (там она теряется), либо ещё одна вкладка в панели (места нет, и панель
// не резиновая).
//
...
// Ненаписанные режимы показаны с подписью «Скоро». Спрятать их до готовности —
// значит показывать оглавление с единственным пунктом, то есть ровно то, что
// было до этой правки. Карточка сразу говорит, что будет внутри.
//
...
// потом формы глагола, потом времена — самое абстрактное в конце.
//
...
const MODES: Mode[] = [
  ...
  },
  {
    ...
    about: "Дан русский перевод — собираешь фразу из слов, а дальше пишешь сам",
    icon: "list",
    fill: ["#8b5cf6", "#6366f1"] as const,
    edge: accents.violetDeep,
    href: "/flashcards/build",
  },
  {
    key: "verbs",
    title: "Неправильные глаголы",
    about: "Вставить нужную форму в предложение: написать или выбрать",
    icon: "repeat",
    fill: gradients.fire,
    edge: "#b45309",
  },
            ...
            <View key={mode.key} style={{ marginBottom: 10 }}>
              ...
              {asked === mode.key && (
                <Text style={{
                  ...
                  проверяются фильтром — пустой тренажёр открывать смысла нет.
                </Text>
              )}
            </View>
          ))}
