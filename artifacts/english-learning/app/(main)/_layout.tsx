    ...
    <>
      ...
      >
        ...
        {/* Рейд-босс: недельное событие на всё сообщество. Стоит сразу за
            «Учёбой» намеренно — это соседние, но РАЗНЫЕ вещи: там учат, здесь
            практикуют по боссу. */}
        <Tabs.Screen
          name="raid"
          options={isStudent
        ...
        {/* Диалоги учителя БОЛЬШЕ НЕ ВКЛАДКА. Создание живёт в «Создать
            задание», сами диалоги и разборы — во вкладке «Задания». Отдельная
            кнопка внизу дублировала бы то, что уже есть, а панель у учителя и
            так набита. Маршрут оставлен и уводит в «Задания»: на него могут
            вести старые ссылки и история браузера. */}
      ...
      {/* Всплывающее окно события. Стоит здесь, а не на конкретном экране:
          ...
          Учителю оно тоже нужно, и ровно ради одного события: ученик прошёл
          диалог. Раньше учитель узнавал об этом, только если сам заходил и
          проверял. Остальные источники ленты у него просто пустые. */}
      {(isStudent || isTeacher) && <NotificationHost />}

      {/* Цифры урона по боссу. Тоже здесь: слой абсолютный и должен лежать
          поверх любого экрана на случай, если ученик открыл бой рейда откуда
          угодно. Реально что-то рисует только во время самого боя
          (POST /raid/answer) — обычные тренажёры «Учёбы» босса больше не
          бьют, см. utils/raidBus.ts. Слой не перехватывает нажатия
          (pointerEvents="none"). */}
