...
//
// ── Очки НЕ мелькают во время занятия ────────────────────────────────────────
// Раньше очки были видны в ТРЁХ местах: счётчик в шапке рос с каждым верным
// ответом, «+N» вспыхивало в вердикте сразу под ответом, и общая сумма ждала на
// итогах захода. Для тренажёра слов уже было решено, что живой счётчик посреди
// занятия отвлекает от самого задания (см. components/WordTrainer.tsx, «Опыт не
// мелькает во время тренировки») — очки копятся молча, а видно их становится
// РОВНО ОДИН РАЗ, на итогах. Здесь было иначе, и это разъезжалось с остальными
// разделами: одно и то же число «опыт за это занятие» то мигало во время игры,
// то нет, в зависимости от того, в каком именно тренажёре ты сейчас находишься.
//
// Теперь и здесь очки нигде не показываются, пока заход не закончен: state
// points по-прежнему копится на каждом submit (он нужен для плитки на итогах),
// но ни в шапке, ни в самом вердикте больше нет счётчика — только на экране
// «Заход закончен».
//
// ── Дневной потолок показывается ТОЛЬКО когда он достигнут ─────────────────
// Формулировкой «на сегодня достаточно», а не отказом. Постоянная строка «12 из
// 30» превратила бы занятие в выработку нормы, а упёршийся в потолок без
// объяснения решит, что что-то сломалось.
//
...
//
// ── Уровень задания в шапке НЕ показывается ─────────────────────────────────
// По задумке грамматики ученик со своим уровнем ВИДИТ вперемешку и задания
// уровня ниже (это повторение, см. fitsLevel()/levelsUpTo() в lib/grammar) —
// то есть A2-ученик обязан встретить задание, помеченное A1. Раньше шапка честно
// писала «уровень A1» — и это читалось как баг («у меня же A2!»), хотя было
// сделано намеренно. Строка убрана целиком: ученику незачем знать уровень
// КОНКРЕТНОГО задания, только прогресс по заходу.
//
// ── Верный ответ ПОСЛЕ ответа показывается фиолетовым — ПРЯМО В ПРЕДЛОЖЕНИИ ──
// Сервер всегда присылает verdict.full — то же самое предложение, но с уже
// подставленным верным словом/словосочетанием (см. checkGrammarAnswer в
// api-server/src/lib/grammar/engine.ts, поле full считается ВСЕГДА, а не только
// при ошибке). Раньше клиент брал verdict.full ТОЛЬКО когда ответ был неверным,
// а при верном ответе снова показывал шаблонный прочерк card.text.replace("___",
// BLANK) — то есть после правильного ответа предложение на долю секунды
// показывало не то, что ученик только что написал, а голый прочерк. Теперь
// verdict.full используется при ЛЮБОМ ответе, верном и неверном.
//
// Кроме того, само предложение раньше красилось целиком одним цветом — было
// непонятно, КАКОЕ именно слово/словосочетание в нём и есть верный ответ,
// особенно в длинном предложении. Теперь locateAnswer() ищет verdict.expected[0]
// как подстроку внутри verdict.full и выделяет её фиолетовым (colors.primary),
// тем же приёмом, что уже используется для ЖИВОГО ввода (liveGap чуть ниже) —
// вложенный Text внутри Text, без ручной вёрстки переноса строк.
//
// ── ГРАБЛИ ──────────────────────────────────────────────────────────────────
...
// единое целое ровно так же, как обычная строка. Выделение верного ответа после
// ответа устроено ТЕМ ЖЕ приёмом — см. locateAnswer/located ниже.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Ищет верный ответ (verdict.expected[0]) ВНУТРИ уже готового предложения
 * (verdict.full) — чтобы выделить фиолетовым именно его, а не красить всё
 * предложение одним цветом. Без учёта регистра: ответ в начале вопроса
 * пишется с заглавной («Does he...»), а expected обычно приходит со строчной.
 *
 * null — подстрока не нашлась (не должно случаться при нормальных данных:
 * verdict.full — это тот же текст с уже подставленным expected[0], см.
 * checkGrammarAnswer в api-server/src/lib/grammar/engine.ts), но рендер не
 * должен падать на нестыковке форматов между разными банками заданий —
 * тогда предложение просто рисуется одним цветом, как раньше.
 */
function locateAnswer(text: string, answer: string): { before: string; match: string; after: string } | null {
  if (!answer) return null;
  const idx = text.toLowerCase().indexOf(answer.toLowerCase());
  if (idx === -1) return null;
  return { before: text.slice(0, idx), match: text.slice(idx, idx + answer.length), after: text.slice(idx + answer.length) };
}
...
function GrammarTrainer({
  ...
  // Копится молча в течение всего захода: см. «Очки НЕ мелькают во время
  // занятия» в шапке файла. Виден он ровно один раз — на экране «Заход
  // закончен».
      ...
      <ScrollView contentContainerStyle={{
        ...
        {answered > 0 ? (
          <>
            ...
            {/* Плитка очков — единственное место, где очки вообще видны (см. «Очки
                НЕ мелькают во время занятия» в шапке файла), и только когда они
                есть: ноль в наградной плитке — это не награда. */}
            {points > 0 && (
  ...
  // именно её и нужно показать после ответа (и верного, и неверного — иначе
  // после верного ответа мелькал бы шаблонный прочерк вместо своего же слова,
  // см. блок «Верный ответ ПОСЛЕ ответа...» в шапке файла).
  const shown = verdict?.full ?? card.text.replace("___", BLANK);

  // Слово/словосочетание, которое и есть верный ответ, — выделяем фиолетовым
  // ВНУТРИ shown (а не красим всё предложение одним цветом). До ответа выделять
  // нечего: verdict ещё нет.
  const highlightAnswer = verdict?.expected?.[0];
  const located = verdict && highlightAnswer ? locateAnswer(shown, highlightAnswer) : null;
  ...
  return (
    ...
    >
      {/* Шапка: выход, название режима, прогресс. Очков здесь больше нет — см.
          «Очки НЕ мелькают во время занятия» в шапке файла: они копятся молча и
          видны один раз, на итогах захода. */}
      <View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 10 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {/* Прогресс по заходу. Уровень задания здесь намеренно не показывается:
              см. «Уровень задания в шапке НЕ показывается» в шапке файла. В
              повторении вместо этого — откуда пришло текущее задание: карточки
              там идут из разных режимов, и это важнее прогресса по номеру. */}
          <Text style={{ fontSize: 11, color: colors.mutedForeground, fontVariant: ["tabular-nums"] }}>
            {isReview ? `${pos + 1} из ${cards.length} · ${FROM[card.mode]}` : `${pos + 1} из ${cards.length}`}
          </Text>
        </View>
      </View>
      ...
      <Tile glow={accents.violetDeep} style={{ padding: 18, marginTop: 16 }}>
        ...
        {card.input === "assemble" ? (
          ...
          <>
            {liveGap ? (
              ...
              // Верный ответ найден внутри предложения — выделяем именно его, а не
              // всю фразу целиком. Тот же вложенный-Text приём, что у liveGap выше.
              <Text style={{ fontSize: 20, fontWeight: "800", color: colors.foreground, textAlign: "center", marginTop: 12, lineHeight: 30 }}>
                {located.before}
                <Text style={{ color: colors.primary, fontWeight: "900" }}>{located.match}</Text>
                {located.after}
              </Text>
            ) : (
              <Text style={{ fontSize: 20, fontWeight: "800", color: colors.foreground, textAlign: "center", marginTop: 12, lineHeight: 30 }}>
        ...
        {/* Разбор ответа. Строка «+N очков» здесь больше не показывается — см.
            «Очки НЕ мелькают во время занятия» в шапке файла: очки копятся молча,
            видны только на итогах захода. */}
        {verdict && (
