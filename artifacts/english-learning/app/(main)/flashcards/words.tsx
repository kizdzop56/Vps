// ─────────────────────────────────────────────────────────────────────────────
// Режим «Слова»: сквозная сессия по всем колодам, отработка сложных слов,
// тематические колоды, собственные колоды и переходы к статистике / созданию
// колоды / марафону.
//
// Раньше этот экран был самой вкладкой. Теперь вкладка — выбор режима
// (app/(main)/flashcards.tsx), а слова живут здесь: у раздела появились
// соседи (конструктор предложений, неправильные глаголы, времена), и держать
// один из режимов на месте оглавления было бы странно.
//
...
// 5. Пилюль-счётчиков (dueCount/newCount) на строке колоды. Синий кружок с
//    числом к повторению и розовый «+N новых» выглядели как игровая валюта
//    или награда — пользователи путали их с опытом/очками, хотя это чисто
//    технический подсчёт состояния карточек. Тот же состав колоды и так есть
//    текстом («начато N · выучено M») и прогресс-баром — двух представлений
...
export default function WordsScreen() {
  ...
  return (
    ...
    >
      {/* Заголовок + уровень. Стрелка ведёт к выбору режима: панель вкладок
          теперь открывает оглавление раздела, а не сразу слова. */}
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
          <Pressable
            onPress={() => router.replace("/flashcards")}
            accessibilityRole="button"
            accessibilityLabel="К выбору режима"
            hitSlop={10}
      ...
      {/* Главное действие. Подпись без цифр: см. «Чего здесь нет» в шапке файла. */}
      <ChunkyButton
        label="Учить слова"
  ...
  return (
    ...
    >
      <View style={cardBody(colors, {
        ...
        {/* Пилюли dueCount/newCount убраны: путали пользователей с очками/опытом,
            см. «Чего здесь нет» в шапке файла. Тот же состав колоды уже виден
            текстом и полосой прогресса выше. */}
        <Glyph name="chevron" size={20} color={colors.mutedForeground} />
      </View>
    </ChunkyTap>
  );
