...
// говорит или просто дышит.
//
// ── ГРАБЛИ: СНЕЖА УЕЗЖАЛА ВМЕСТЕ С ЛЕНТОЙ ──────────────────────────────────
// Экран был одним большим ScrollView: заголовок, Снежа, переключатель режима и
// лента реплик — всё внутри. После пяти реплик лента доросла до края,
// автопрокрутка ушла вниз, и Снежа осталась выше экрана.
//
// Теперь экран — ТРИ элемента одного flex-столбца: шапка (не прокручивается),
// лента (flex: 1, прокручивается она одна) и панель ввода (не прокручивается).
// Поэтому Снежа видна ВСЕГДА, сколько бы реплик ни было.
//
// ── ГРАБЛИ: ЗВУК В ОТВЕТЕ РВАЛ ЗАПРОС НА МОБИЛЬНОЙ СЕТИ ────────────────────
// Ответ на реплику раньше приходил вместе с готовой озвучкой — несколько сотен
// килобайт base64 внутри JSON. На 3G такой запрос обрывался, и Safari сообщал
// ровно «Load failed». Со стороны это выглядело как «диалог сломался».
//
// Теперь сервер отдаёт только текст (см. ГРАБЛИ в routes/voiceChat.ts), а звук
// экран просит отдельно — сразу после появления реплики. Реплика видна
// немедленно, голос догоняет её через секунду.
//
// ── ОШИБКА СЕТИ ОБЪЯСНЯЕТСЯ ПО-ЧЕЛОВЕЧЕСКИ ─────────────────────────────────
// Браузер отдаёт «Load failed» (Safari) или «Failed to fetch» (Chrome). Для
// ребёнка это шум, и хуже того — по нему нельзя понять, что делать. Такие
// сообщения переводятся в одну понятную фразу про интернет, а сама реплика
// получает кнопку «Отправить ещё раз».
//
// Кнопка важнее текста: без неё недоставленная реплика оставалась в ленте
// мёртвым грузом, и разговор действительно ломался — приходилось набирать всё
// заново.
//
// ── ОШИБКА В ЯЗЫКЕ ОСТАНАВЛИВАЕТ РАЗГОВОР ──────────────────────────────────
// Снежа проверяет каждую реплику. Ошиблась фраза — под ней появляется разбор
// (как правильно и что не так), Снежа просит повторить, и над кнопкой висит
// полоса «повтори правильно». Разговор дальше не идёт, пока фраза не прозвучит
// верно. Русская реплика — такой же случай: в разборе приходит её английский
// перевод.
//
// Полоса нужна отдельно от реплики: ленту прокрутили — и просьба уехала вверх,
// а руки ученика внизу, у кнопки.
//
...
//
// Свёрнут ПО УМОЛЧАНИЮ: перевод, который висит рядом всегда, убивает смысл
// упражнения — глаз читает русское и до английского просто не доходит.
//
// ── ГРАБЛИ: ЗАПРЕТ АВТОЗАПУСКА ЗВУКА ───────────────────────────────────────
// Ответ приходит через несколько секунд после нажатия, и к этому моменту Safari
// уже не считает проигрывание следствием действия человека. Лечится в
// utils/voiceRecorder.ts: плеер один и разблокируется в момент нажатия
// (primeAudio), поэтому primeAudio ОБЯЗАН вызываться синхронно в обработчике,
// до любого await.
//
// ── СВОЯ РЕПЛИКА ПОКАЗЫВАЕТСЯ СРАЗУ ────────────────────────────────────────
// Она встаёт в ленту ДО запроса, а по ответу сервера либо уточняется
...
export default function TutorScreen() {
  ...
  // Лента начинается ПУСТОЙ: первым говорит ученик.
   ...
   * не приезжает, и это единственный путь к голосу (см. шапку файла).
   *
   * @param silentFail не показывать пометку об ошибке. Нужно для
   *        автоматической озвучки: если голос не пришёл сам, ученик всё равно
   *        может нажать на реплику, и вот тогда отказ стоит показать.
   */
  const speakLine = React.useCallback(async (line: Line, silentFail = false) => {
    ...
    try {
      ...
      if (!silentFail) {
        // Тихо и на самой реплике: плашка с ошибкой тут слишком громкая для
        // того, что текст ответа уже прочитан.
    ...
    >
      ...
      >
        ...
        {/* Ошибка отправки. Вместе с ней — кнопка повтора: без неё
            недоставленная реплика остаётся мёртвым грузом, и разговор
