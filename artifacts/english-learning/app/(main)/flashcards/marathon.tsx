...
// Единственный возврат во вкладку «Слова» — стрелка в шапке. Раньше на пустом
// марафоне (у новичка это первый же заход) стояла большая кнопка «Учить слова»,
// которая вела туда же: единственное действие экрана возвращало ровно туда,
// откуда пришёл, и всё вместе читалось как «кнопка марафона выкидывает назад».
// Теперь эта кнопка ведёт прямо в сессию.
//
 ...
 * Снаружи это выглядит как «нажимаю Марафон, меня выкидывает в Слова», и
...
export default function MarathonScreen() {
  ...
  // Выход во вкладку «Слова» задан явным адресом, а не router.back(). В
  // навигации по вкладкам «назад» возвращает на ПЕРВУЮ вкладку («Задания»), а
  // не на предыдущий экран. Панель вкладок здесь скрыта, поэтому стрелка в
  // шапке — единственный выход, и других путей назад на экране быть не должно.
  const backToWords = React.useCallback(() => {
    router.replace("/flashcards");
  }, [router]);

  /** Пустой зал повторений отправляет учиться, а не обратно во вкладку. */
      ...
      {q.isLoading ? (
        ...
        <>
          ...
          {data.totalWords === 0 ? (
            ...
            // Повторять нечего. Кнопка ведёт УЧИТЬСЯ, а не обратно во вкладку:
            // единственное действие экрана не должно возвращать туда, откуда
            // пришёл, — со стороны это неотличимо от «выкинуло назад».
