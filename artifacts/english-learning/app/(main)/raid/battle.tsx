...
//
// ── Пропуск в тексте задания заполняется вживую, а после ответа — верным словом
...
// двух других тренажёрах (см. их шапки за подробным обоснованием).
//
// ПОСЛЕ ответа пропуск раньше просто исчезал вместе с TextInput, а предложение
// откатывалось на task.prompt — то есть на СЫРОЙ шаблон с подчёркиваниями
// вместо слова: и верно ответивший, и ошибившийся видели одну и ту же дыру на
// месте ответа, хотя verdict.expected уже пришёл с сервера. Теперь после ответа
// пропуск заполняется verdict.expected[0] (или тем, что написал игрок, если по
        ...
        <View style={cardStyle(colors)}>
          {task.listen ? (
            ...
            <Text style={{
              ...
              {answerGap.before}
              <Text style={{ color: colors.primary }}>{answerWord}</Text>
              {answerGap.after}
            </Text>
          ) : (
            <Text style={{
