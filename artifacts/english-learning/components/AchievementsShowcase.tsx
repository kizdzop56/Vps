...
//
// В веере медали не нажимаются по отдельности и не подписаны: он про «вот что
// я собрал», а не про справочник. Подписи живут в полном списке.
//
...
//
// ── Полный список: мелкие медали с названиями ───────────────────────────────
// В списке медаль — не экспонат, а строка каталога: важно быстро найти нужную
// и понять, что это вообще за награда. Поэтому здесь она мельче веера
// (GRID_SIZE) и подписана названием в две строки.
//
// Раньше было наоборот: подписи убрали, а медали раздули до 76 пикселей. Из-за
// этого 50 наград превращались в бесконечную ленту одинаковых кружков без
// единого слова — чтобы узнать имя любой, приходилось открывать карточку.
//
// ── ГРАБЛИ: сетка и gap ─────────────────────────────────────────────────────
// Ячейки НЕ используют gap вместе с процентной шириной. Было `width: "31%"`
// плюс `gap: 12`: на телефоне 3 × 31 % + 2 × 12 px оказывалось на пару
// пикселей шире контейнера, третья медаль переносилась, и вместо трёх в ряд
// выходило две с огромной дырой справа. Теперь ширина ровно 1/3, а промежутки
// делает внутренний padding самой ячейки — арифметика сходится на любом экране.
//
...
/**
 * Медаль в полном списке. Мельче веера: под ней стоит название, и вместе они
 * должны читаться одной ячейкой каталога, а не двумя объектами.
 */
const GRID_SIZE = 54;
...
/**
 * Ячейка полного списка: медаль и её название в две строки.
 *
 * Название нужно именно здесь: список — это каталог, и «что это за кружок»
 * должно быть понятно без открытия карточки. Полное описание и условие
 * получения по-прежнему живут в карточке по тапу.
 *
 * У полученной медали название тёмное и жирное, у закрытой — приглушённое:
 * собранное должно читаться первым.
 */
function BadgeCard({
  achievement,
  isLocked = false,
  ...
  return (
    ...
    >
      <View>
        ...
        {!isLocked && (
          <View style={{
            position: "absolute", left: 0, right: 0, top: 3, height: GRID_SIZE,
        ...
        <Animated.View style={{ transform: [{ translateY: press }] }}>
          ...
          >
            {isLocked ? (
              <Glyph name="lock" size={21} color="rgba(91,79,142,0.5)" />
            ) : (
              <MedalFace achievement={achievement} size={GRID_SIZE} glyphSize={25} />
            )}
          </View>
        </Animated.View>
        <View style={{ height: 3 }} />
      ...
      {/* Название. Две строки — потолок: третья уже съедает высоту ряда, а
          длинные имена и так читаются по первым словам. */}
      <Text
        numberOfLines={2}
        style={[
          styles.badgeTitle,
  ...
  return (
    <View style={styles.wrap}>
      ...
      {/* ── Полный список: раскрывается на месте ──
          Только здесь медали нажимаются: тап открывает карточку с описанием и
          условием получения. */}
      {expanded && (
...
const styles = StyleSheet.create({
  ...
  // БЕЗ gap: промежутки делает padding ячейки. Проценты плюс gap не сходятся
  // по ширине, и третья медаль переносится на новую строку (см. ГРАБЛИ).
  grid: { flexDirection: "row", flexWrap: "wrap" },
  // Ровно треть ширины: три медали в ряд на любом экране.
  badgeWrap: {
    width: "33.333%", alignItems: "center",
