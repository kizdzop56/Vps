...
// огонь, звезда, вспышка, сердце, play. Цвет берётся из пропа color.
//
// ── Про темы колод ──────────────────────────────────────────────────────────
// Вторая половина набора (home, car, plane, shirt, ball, cart, palette, heart,
// cloud, hash, utensils, tools, flask, building, phone) добавлена ради раздела
// «Слова»: там у каждой колоды своя тема, и без этих значков половина списка
// показывала не иконку, а первую букву названия в цветной плашке. Сопоставление
// «тема → глиф» живёт в components/ui/DeckGlyph.tsx.
//
// ── «check» — ПРОСТАЯ галочка, без квадрата вокруг ──────────────────────────
// Раньше «check» рисовал галочку ВНУТРИ скруглённого квадрата (второй путь
// дорисовывал рамку вроде блокнота/чек-листа). Сам по себе значок смотрелся
// нормально, но там, где под него подкладывали цветной круглый бейдж (а таких
// мест в приложении много — итог ответа, «формы выучены», значки в рядах
// списков), получался «квадрат с галочкой в кружке» — двойная рамка, которую
// читали как чекбокс, а не как «готово/верно». Теперь это ОДИН штрих —
// обычная галочка, отцентрованная в сетке 24×24, без какой-либо рамки вокруг.
// Раз Glyph — единственный источник иконок в приложении (второго набора нигде
// в репозитории нет), эта правка чинит ВСЕ места использования name="check"
// разом.
// ─────────────────────────────────────────────────────────────────────────────
...
export type GlyphName =
  ...
  // темы колод
  | "home" | "car" | "plane" | "shirt" | "ball" | "cart" | "palette"
  | "heart" | "cloud" | "hash" | "utensils" | "tools"
  | "building" | "phone" | "flask";
...
function render(name: GlyphName, s: any, color: string) {
  switch (name) {
    ...
    case "check":
      // Простая галочка одним штрихом — без рамки вокруг (см. заголовок файла).
      // Отцентрована в сетке 24×24, а не смещена к правому краю: раньше её
      // положение считалось от рамки, которой больше нет.
      return <Path {...s} strokeWidth={2.6} d="M20 6.4 9.6 17 4.6 12.1" />;
    ...
    case "home":
      return (
        <G>
          <Path {...s} d="M3.4 10.4 12 3.2l8.6 7.2v9a1.8 1.8 0 0 1-1.8 1.8H5.2a1.8 1.8 0 0 1-1.8-1.8v-9Z" />
          <Path {...s} d="M9.6 21.2v-6.4h4.8v6.4" />
        </G>
      );
    ...
    case "car":
      return (
        <G>
          <Path {...s} d="M3.6 14.4 5.4 9.3a2.2 2.2 0 0 1 2.08-1.5h9.04a2.2 2.2 0 0 1 2.08 1.5l1.8 5.1v3.4a1 1 0 0 1-1 1h-1.6a1 1 0 0 1-1-1v-1H7.2v1a1 1 0 0 1-1 1H4.6a1 1 0 0 1-1-1v-3.4Z" />
          <Path {...s} d="M3.6 14.4h16.8" />
          <Circle cx={7.4} cy={17} r={1.2} fill={color} />
          <Circle cx={16.6} cy={17} r={1.2} fill={color} />
        </G>
      );
    ...
    case "plane":
      return (
        <Path
          d="M10.6 3a1.4 1.4 0 0 1 2.8 0v5.5l7.4 4.3v2.4l-7.4-2.3v4.3l2.4 1.8v1.8L12 19.6l-3.8 1.2v-1.8l2.4-1.8v-4.3L3.2 15.2v-2.4l7.4-4.3V3Z"
          fill={color}
        />
      );
    ...
    case "shirt":
      return (
        <G>
          <Path {...s} d="M8.8 3.2 4.2 5.4l1.4 4.3 2.2-.7v10.4a1.4 1.4 0 0 0 1.4 1.4h5.6a1.4 1.4 0 0 0 1.4-1.4V9l2.2.7 1.4-4.3-4.6-2.2" />
          <Path {...s} d="M8.8 3.2a3.2 3.2 0 0 0 6.4 0" />
        </G>
      );
    ...
    case "ball":
      return (
        <G>
          <Circle {...s} cx={12} cy={12} r={8.8} />
          <Path {...s} d="m12 7.4 3.5 2.5-1.35 4.1h-4.3L8.5 9.9 12 7.4Z" />
          <Path {...s} d="M12 3.2v4.2M4.6 9.6l3.9.3M19.4 9.6l-3.9.3M7.6 19.4l2.2-5.4M16.4 19.4l-2.2-5.4" />
        </G>
      );
    ...
    case "cart":
      return (
        <G>
          <Path {...s} d="M2.6 3.6h2.5l2.4 11.2h9.9l2-7.6H6.2" />
          <Circle cx={9.4} cy={19} r={1.6} fill={color} />
    ...
    case "palette":
      return (
        <G>
          <Path {...s} d="M12 3.2c-4.86 0-8.8 3.94-8.8 8.8s3.94 8.8 8.8 8.8a1.9 1.9 0 0 0 1.9-1.9c0-.5-.19-.94-.5-1.27a1.86 1.86 0 0 1-.5-1.27c0-1.05.85-1.9 1.9-1.9h2.24A4.16 4.16 0 0 0 21.2 10.4C21.2 6.44 17.08 3.2 12 3.2Z" />
          <Circle cx={7.6} cy={11.6} r={1.3} fill={color} />
          <Circle cx={10.6} cy={7.6} r={1.3} fill={color} />
          <Circle cx={15.4} cy={8.4} r={1.3} fill={color} />
        </G>
      );
    ...
    case "heart":
      return (
        <Path
          d="M12 20.8 4.5 13.3a4.9 4.9 0 0 1 6.93-6.93l.57.57.57-.57a4.9 4.9 0 0 1 6.93 6.93L12 20.8Z"
          fill={color}
        />
      );
    ...
    case "cloud":
      return (
        <Path {...s} d="M7.2 18.8a4.5 4.5 0 0 1-.5-8.97 5.7 5.7 0 0 1 11.02-1.16A4.3 4.3 0 0 1 17.4 18.8H7.2Z" />
      );

    case "hash":
      return <Path {...s} d="M9.6 3.4 7.8 20.6M16.4 3.4l-1.8 17.2M3.6 8.8h16.8M2.8 15.4h16.8" />;

    case "utensils":
      return (
        <G>
          <Path {...s} d="M6.2 2.8v5.4M9.4 2.8v5.4M12.6 2.8v5.4" />
          <Path {...s} d="M6.2 8.2h6.4a3.2 3.2 0 0 1-3.2 3.6v9.4" />
          <Path {...s} d="M18 2.8c1.6 2.1 2.4 4.4 2.4 6.9 0 1.9-.8 3-2.4 3.3v8.2" />
        </G>
      );
    ...
    case "tools":
      return (
        <Path
          {...s}
          d="M20.2 4.6 17.4 7.4l-1.4-.3-.3-1.4 2.8-2.8a5 5 0 0 0-6.6 6.1L4.3 16.6a2.2 2.2 0 0 0 3.1 3.1l7.6-7.6a5 5 0 0 0 6.1-6.6l-.9-.9Z"
        />
      );
    ...
    case "flask":
      return (
        <G>
          <Path {...s} d="M9.4 2.8h5.2" />
          <Path {...s} d="M10.4 2.8v6.2l-5.5 9a2 2 0 0 0 1.7 3h10.8a2 2 0 0 0 1.7-3l-5.5-9V2.8" />
          <Path {...s} d="M7.4 14.8h9.2" />
        </G>
      );
    ...
    case "building":
      return (
        <G>
          <Path {...s} d="M3.6 21.2V6.4a1.6 1.6 0 0 1 1.6-1.6h5.2a1.6 1.6 0 0 1 1.6 1.6v14.8" />
          <Path {...s} d="M12 10.4h6.8a1.6 1.6 0 0 1 1.6 1.6v9.2" />
          <Path {...s} d="M2.4 21.2h19.2M6.4 8.6h2.8M6.4 12.4h2.8M6.4 16.2h2.8M15 14h2.4M15 17.6h2.4" />
        </G>
      );
    ...
    case "phone":
      return (
        <G>
          <Rect {...s} x={6.4} y={2.4} width={11.2} height={19.2} rx={2.8} />
          <Path {...s} d="M10.2 18.6h3.6" />
        </G>
      );
