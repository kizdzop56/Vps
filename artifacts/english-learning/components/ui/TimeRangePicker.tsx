...
//  2. У каждой из четырёх колонок была своя подсветка — на экране четыре
...
//  3. Колонки были по 52 пикселя в пять строк: блок занимал 260 пикселей.
//     Три строки по 44 — 132, вдвое меньше, а прокрутка та же.
//
// ── Минуты по одной ─────────────────────────────────────────────────────────
// Барабан минут выдаёт ВСЕ 60 значений, а не сетку по пять. Шаг в пять минут
// смотрелся аккуратно ровно до первого живого случая: занятие в 11:22, потому
// что ученик освобождается в 11:20, а до компьютера идти две минуты. Такое
// время просто нельзя было выбрать — приходилось врать расписанию.
//
// Шестьдесят строк вместо двенадцати колесо не портят: снап тот же, а долгую
// прокрутку решает тап по нужной строке — она выбирает значение сразу.
//
// Сверху строка итога «13:00 → 14:00» с длительностью: раньше результат
// приходилось складывать глазами из разных колонок.
// ─────────────────────────────────────────────────────────────────────────────
...
/** Все минуты часа: занятие может начаться в 11:22, и это нормально. */
 ...
 *
 * Тап по строке выбирает значение сразу — с шестьюдесятью минутами это
 * единственный быстрый способ добраться до дальнего числа, не мотая колесо.
 */
function Column({
  ...
  return (
    <View style={{ flex: 1, alignItems: "center" }}>
      ...
      <View style={{ position: "relative", flexDirection: "row", alignItems: "center" }}>
        ...
        <Column items={MINUTES} value={m} onChange={(nm) => onChange(h * 60 + nm)} width={52} />
      </View>
    </View>
  );
}
