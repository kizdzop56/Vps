...
// КРУПНОЙ и наливается от нуля до конца, а на её месте появляется новый номер.
// То, что в шапке выглядело потерей, здесь выглядит тем, чем является.
//
// ── Порядок кадров важен ────────────────────────────────────────────────────
// Всё происходит по очереди, а не разом:
//
//   1. затемнение и подъём карточки        260 мс
//   2. полоса РАСТЁТ в размере (springs)   короткий рывок: она «выходит
...
//
// ── Свечение ────────────────────────────────────────────────────────────────
// Три круга под значком: расходятся и гаснут по кругу. Радиального градиента в
// expo-linear-gradient нет, а тень в RN не размывается настолько сильно —
// поэтому свечение собрано из полупрозрачных окружностей. Это дешёвый приём, но
// на движении он читается именно как свет.
//
// Цвет свечения — цвет НОВОГО уровня из constants/xpLevels.ts. Своего цвета у
// окна нет намеренно: у пятидесяти уровней своя палитра, и она здесь главная.
//
// ── Про useNativeDriver ─────────────────────────────────────────────────────
...
import {
  View, Text, Modal, Animated, Easing, StyleSheet, Platform,
  useWindowDimensions,
} from "react-native";
...
export function LevelUpCelebration({ visible, fromLevel, toLevel, onClose }: Props) {
  ...
  /** Один круг свечения. Задержка сдвигает волну, чтобы они шли друг за другом. */
