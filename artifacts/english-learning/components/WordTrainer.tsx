...
// информация о слове (слово, транскрипция, перевод, часть речи, пример) вместе
// с итогом ответа (верно/неверно, что было правильно, кнопка «Прослушать»).
//
...
//
// ── Слов без иллюстрации-эмодзи ──────────────────────────────────────────────
// У части слов в данных есть поле emoji (например 🍞 у bread, 🍎 у apple), но
// в тренажёре оно нигде не рисуется — ни на знакомстве, ни на обратной стороне
// ответа. У большинства слов этого поля нет, и карточка «то с картинкой, то
// без» смотрелась неровно; теперь все карточки выглядят одинаково независимо
// от того, заполнено это поле в конкретном слове или нет. Само поле в данных
// не трогали — только рендер.
//
// ── Шрифт слова на КАРТОЧКЕ подстраивается под его длину — НО ЩАДЯЩЕ ────────
...
//
// Пороги настроены так, чтобы короткие и средние по длине слова/фразы
// (примерно до 12 символов — это покрывает подавляющее большинство обычных
// слов вроде «apartment», «furniture», «breakfast») вообще НЕ ужимались и
// держали один и тот же размер, что и у прочих карточек. Раньше порог начинал
// действовать уже с 8 символов, и «apartment» (9 букв) неоправданно терял 15%
// размера рядом с короткими словами — карточки одной колоды визуально не
// совпадали, хотя реальной причины сжимать шрифт не было: слово прекрасно
// умещается в одну строку.
//
// Настоящий риск переполнения — только у длинных ИДИОМ («there's no place
// like home») и у по-настоящему длинных одиночных слов («выздоравливать»).
// Для них шрифт по-прежнему уменьшается — иначе воспроизводится старый баг
// (текст обрезался многоточием, потому что adjustsFontSizeToFit ненадёжен для
// многострочного текста именно на react-native-web, это же веб-сборка). Запас
// на такой случай — увеличенный numberOfLines (4 вместо 3): у длинной идиомы
// теперь больше места перенестись по строкам ДО того, как придётся ужимать
// шрифт.
//
// Для ОДНОГО слова оба числа (длина слова и длина всей фразы) совпадают,
// поэтому подбор размера для одиночных слов теперь почти всегда даёт fontSize
// без изменений — правило ужимает только настоящие длинные фразы и слова.
//
...
//
// ── НО: предложение с пропуском — НЕ «одна строка без права на перенос» ────
// У письменного упражнения (gapExercise в api-server/wordExercise.ts) prompt
// это НЕ одиночное слово/фраза с карточки — это ПОЛНОЕ ПРЕДЛОЖЕНИЕ с пропуском
// плюс перевод второй строкой, склеенные через "\n\n" в одну строку:
// «Communication is an important skill.\n\nОбщение — важный навык.». Раньше
// fitFontSize мерил эту строку ЦЕЛИКОМ как «фразу без переноса» (тем же
// правилом, что для одного слова на карточке) — а такая строка почти всегда
// набирает 50–70+ символов суммарно, что уводило шрифт до нечитаемых ~14px,
// хотя само предложение прекрасно переносится на отведённые 4 строки, места
// более чем достаточно.
//
// Отличить этот случай легко: если в prompt есть перенос строки (\n), значит
// это НЕ одиночное слово с карточки, а нормальный текст, который ПЕРЕНОСИТСЯ
// по словам, — то есть ровно тот случай, для которого уже существует
// fitOptionFontSize (меряет по самому длинному ОДНОМУ слову, а не по сумме).
// fitFontSize при наличии \n отдаёт расчёт именно ей — см. её тело ниже.
//
...
//
// ── Карточка с пустым словом никогда не показывается ────────────────────────
// Если у слова пустое поле english (испорченная запись в колоде — например,
// слово когда-то добавили только с переводом, без английского текста), сервер
// всё равно может прислать под него intro-упражнение: тип "intro", prompt —
// само card.english, то есть пустая строка. Раньше это рисовало карточку
// БЕЗ СЛОВА ВООБЩЕ: пустое место наверху, только кнопка «Прослушать» и
// «Показать перевод» — выглядело как баг ни на что не похожий, потому что
// текста в буквальном смысле не было. Теперь такие карточки отфильтровываются
// ещё на загрузке очереди (см. эффект загрузки ниже) — слово без английского
// текста в сессию просто не попадает, вместо того чтобы рисоваться сломанным.
//
...
// Эмодзи в интерфейсе не используются в принципе — ни в интерфейсе, ни в
// иллюстрации к слову (см. блок выше).
//
...
/**
 * Сколько строк отводится под слово/фразу на лицевой стороне карточки.
 *
 * Увеличено с 3 до 4 вместе с ослаблением порогов fitFontSize (см. заголовок
 * файла): раз шрифт теперь ужимается реже, длинной идиоме иногда нужно больше
 * строк для переноса вместо уменьшения кегля.
 */
 ...
 *
 * Пороги подняты специально, чтобы короткие и средние слова/фразы (примерно
 * до 12 символов — «apartment», «furniture», «breakfast» и почти все обычные
 * слова) НЕ теряли размер вовсе: раньше порог начинался с 8 символов, и любое
 * слово чуть длиннее среднего визуально «мельчало» рядом с короткими словами
 * без реальной необходимости — одно слово почти никогда не рискует не
 * поместиться в отведённые строки. Ужимается по-настоящему только то, что
 * рискует не поместиться даже в PROMPT_MAX_LINES строк: длинные идиомы
 * («there's no place like home») и по-настоящему длинные одиночные слова
 * («выздоравливать»). adjustsFontSizeToFit, который должен был доужать шрифт
 * дальше сам, на вебе (react-native-web) ненадёжен именно для многострочного
 * текста — отсюда и предпросчитанные пороги вместо расчёта на глаз рантаймом.
 *
 * Для ОДНОГО слова оба числа (длина слова и длина всей фразы) совпадают —
 * подавляющее большинство обычных слов проходит без изменений.
 *
 * ИСКЛЮЧЕНИЕ: если в text есть перенос строки (\n) — это НЕ одиночное слово/
 * фраза карточки, а предложение с пропуском + перевод, склеенные через "\n\n"
 * (см. gapExercise в api-server/wordExercise.ts). Такой текст ПЕРЕНОСИТСЯ по
 * словам на несколько строк, и мерить его по суммарной длине ВСЕЙ строки
 * неправильно ровно по той же причине, по которой варианты ответа меряются
 * отдельной функцией (fitOptionFontSize) — см. блок в шапке файла «НО:
 * предложение с пропуском...». В этом случае fitFontSize просто делегирует
 * расчёт fitOptionFontSize (по самому длинному ОДНОМУ слову).
 *
 * ИСПОЛЬЗУЕТСЯ для слова/фразы на карточке (exercise.prompt, card.english),
 * для собранного слова в упражнении «Собери слово», и (через делегирование
 * выше) для предложения с пропуском в письме. НЕ используется для перевода
 * (там фиксированный размер, см. INTRO_TRANSLATION_FONT_SIZE/
 * ANSWER_TRANSLATION_FONT_SIZE выше).
 */
function fitFontSize(text: string, base: number): number {
  if (text.includes("\n")) {
    // Предложение с пропуском + перевод — переносится по словам, а не одна
    // неразрывная строка. Меряем как варианты ответа: по самому длинному
    // слову, а не по сумме длины всей строки (см. шапку функции и файла).
    return fitOptionFontSize(text, base);
  }
  ...
  let scale = 1;
  if (effective > 30) scale = 0.42;
  else if (effective > 22) scale = 0.6;
  else if (effective > 16) scale = 0.78;
  else if (effective > 12) scale = 0.9;
 ...
 *
 * Используется также fitFontSize — для предложений с переносом строки (см. её
 * тело выше): по той же причине (текст переносится, а не стоит одной строкой,
 * поэтому меряем по слову, а не по сумме).
 *
 ...
 * api-server/src/lib/wordExercise.ts равен 4 символам, но полагаться на
  ...
  React.useEffect(() => {
    ...
    loader()
      .then((q) => {
        if (!alive) return;
        setQueue(q);
        // Карточка с пустым english (испорченная запись слова — например, его
        // когда-то добавили только с переводом) не отфильтровывалась раньше и
        // рисовалась ПОЛНОСТЬЮ ПУСТОЙ: сервер собирал под неё intro-упражнение
        // с prompt = "" — ни слова, ни ошибки, просто дыра на экране. Отсекаем
        // такие карточки здесь же, при загрузке, а не рендерим их сломанными.
        const raw = q.cards ?? [];
        const usable = raw.filter((c) => !!(c?.english ?? "").trim());
        const next = usable.map((c) => ({ card: c, retry: false }));
        itemsRef.current = next;
