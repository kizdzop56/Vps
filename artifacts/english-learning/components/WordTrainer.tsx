...
//
// fitFontSize используется ТОЛЬКО для самого слова/фразы на карточке (поле
// exercise.prompt, card.english) и для сборки слова из букв — там перенос по
// строкам либо невозможен, либо бессмысленен. Для вариантов ответа своя
// функция (fitOptionFontSize ниже), а для ПЕРЕВОДА — фиксированный размер (см.
// следующий блок), не fitFontSize.
//
// ── Перевод на карточке — ФИКСИРОВАННЫЙ размер, а не подгонка под длину ─────
// Перевод (введение → обратная сторона, и обратная сторона любого ответа)
// раньше считал себе кегль через fitFontSize — и «вода» (одно короткое слово)
// рисовалась полным размером, а «проще простого, пустяк» — заметно мельче,
// хотя это один и тот же элемент интерфейса на разных карточках. Разница была
// не оправдана: перевод и так переносится на несколько строк (numberOfLines=
// {3}), длина текста — это повод перенести строку, а не менять кегль от
// карточки к карточке. Теперь размер перевода — INTRO_TRANSLATION_FONT_SIZE
// (знакомство) и ANSWER_TRANSLATION_FONT_SIZE (обратная сторона ответа) —
// константы, одинаковые для абсолютно всех карточек. adjustsFontSizeToFit и
// minimumFontScale остаются как аварийный тормоз для по-настоящему длинных
// переводов, которые не поместятся и в три строки, — но это исключение, не
// правило: типичный перевод (одно слово, короткая фраза, идиома) теперь везде
// выглядит одинаково, тем же принципом «общий заранее известный размер», что
// уже применён к вариантам ответа (см. следующий комментарий).
//
...
/**
 * Фиксированный размер шрифта перевода — одинаковый для ВСЕХ карточек.
 *
 * INTRO — обратная сторона знакомства (перевод под словом крупно).
 * ANSWER — обратная сторона любого ответа (перевод под словом помельче, там
 * же транскрипция, часть речи и пример).
 *
 * Раньше оба считались через fitFontSize и «плавали» от карточки к карточке
 * в зависимости от длины конкретного перевода — см. заголовок файла.
 */
const INTRO_TRANSLATION_FONT_SIZE = 24;
const ANSWER_TRANSLATION_FONT_SIZE = 20;
 ...
 *
 * ИСПОЛЬЗУЕТСЯ ТОЛЬКО для слова/фразы на карточке (exercise.prompt,
 * card.english) и для собранного слова в упражнении «Собери слово». НЕ
 * используется для перевода (там фиксированный размер, см.
 * INTRO_TRANSLATION_FONT_SIZE/ANSWER_TRANSLATION_FONT_SIZE выше) и НЕ для
 * вариантов ответа — там своя функция, fitOptionFontSize ниже: у вариантов и
 * перевода перенос строки не проблема, а нормальный сценарий, и мерить их по
 * длине ВСЕЙ ФРАЗЫ значит зря мельчить короткие тексты из-за длинных соседей.
 */
  ...
  const introBack = (
    <View style={{ width: "100%", alignItems: "center" }}>
      ...
      {!!introTranslationText && (
        <Text
          style={{
            // Фиксированный размер для ВСЕХ карточек — см. заголовок файла и
            // комментарий у константы INTRO_TRANSLATION_FONT_SIZE. НЕ
            // fitFontSize: тот мельчит длинные переводы, и «вода» с «проще
            // простого, пустяк» рисовались бы разным кеглем.
            fontSize: INTRO_TRANSLATION_FONT_SIZE,
            lineHeight: fitLineHeight(INTRO_TRANSLATION_FONT_SIZE),
  ...
  const answerBack = verdict && !feedback?.retryBuild ? (
    <>
      ...
      {!!card?.translationsRu?.length && (
        <Text
          style={{
            // Тот же фиксированный принцип, что у INTRO_TRANSLATION_FONT_SIZE
            // выше — размер перевода не должен зависеть от его длины.
            fontSize: ANSWER_TRANSLATION_FONT_SIZE,
            lineHeight: fitLineHeight(ANSWER_TRANSLATION_FONT_SIZE),
